...
def Extracao_CNAE(file:str = None, diretorio:str = f'{current_dir}/'):
    ...
    # Lendo o arquivo usando polars
    pl_options = pl.ScanOptions(
        delimiter=';',
        encoding='ISO-8859-1',
        ignore_errors=True,
        columns=CNPJ['ESTABELE'],
        has_headers=False,
        dtype=dtypes,
    )
    chunk_dados = pl.scan_csv(f"{diretorio}/{file}", pl_options, batch_size=1000000)

    # Loop por pedados usando Polars
    contagem_5612100 = 0 
    contagem_5611201 = 0
    contagem_5611203 = 0
    contagem_5611204 = 0
    ...
    for dados in chunk_dados:
        ...
        dados.rename(CNPJ['ESTABELE'], in_place=True)
        logging.info(f'Leitura inicial: {len(dados)}')

        # Replace valores missing para '--empty--'
        dados = dados.fill_nulls('--empty--')

        # Filter nos dados
        dados = dados.filter(pl.col('SITUACAO_CADASTRAL').isin(['02', '03', '04']))
        dados = dados.select(pl.col('*')).drop_nulls()
        logging.info(f'Somente os ativos: {len(dados)}')

        for cnae in lista_cnae:
            # Separando o dataframe com base nos códigos CNAEs
            df_cnae = dados[pl.col('CNAE_PRINCIPAL') == cnae]
            # Concatenando os dados de Telefone e Fax
            df_cnae['TELEFONE1'] = "+55" + df_cnae['DDD1'].cast(str) + df_cnae['TELEFONE1'].cast(str)
            df_cnae['TELEFONE2'] = "+55" + df_cnae['DDD2'].cast(str) + df_cnae['TELEFONE2'].cast(str)
            df_cnae['FAX'] = "+55" + df_cnae['DDD_FAX'].cast(str) + df_cnae['FAX'].cast(str)

            # Dropando os DDDs após a concatenação
            df_cnae = df_cnae.drop('DDD1').drop('DDD2').drop('DDD_FAX')
            # Mudando o tipo de dado da coluna 'CEP' para string
            df_cnae['CEP'] = df_cnae['CEP'].astype(str)
            # Adicionando hífen à coluna 'CEP'
            df_cnae['CEP'] = df_cnae['CEP'].str.slice(0, 5) + '-' + df_cnae['CEP'].str.slice(5, None)
            ...
            if cnae == 5612100:
                ...
                pass
            # Exporta como CSV
            nome_arquivo = f"{Bases_CNAES}/{CNAES[cnae]}.csv"
            df_cnae.to_csv(nome_arquivo, delimiter=';', encoding='utf-8', with_headers=False)
